...
function getUsersDetails()
{
  ...
  $sql = "SELECT user.user_id, user.name, user.email, user.username,
  user.us_phone_num, user.us_faculty, 
  level.level_desc
  FROM user
  JOIN level ON user.level_id = level.level_id
  WHERE user.level_id = 2";
  $result = mysqli_query($conn, $sql);
  return $result;
}
...
function dispUsersDetails($userid, $username, $name, $email, $phone, $faculty)
{
  $element = '
  <form method="post" action="../include/delete.inc.php">
    <tbody>
